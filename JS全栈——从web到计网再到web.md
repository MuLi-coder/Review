...
曾经认真做的复盘，似乎觉得自己已经理解了全栈的数据流，然而近来因Agent部署的事情，让我不得不重新审视自己的知识，似乎我还并没有完整的刻画出Web世界的图景。

然而经过了暑假的那一场意外之后（[[计算机网络初识 —— “一次意外的拒绝”]]）,我似乎尝试对计算机网络的知识进行了一些研究，至少打通了这个网络世界传输信息的具体过程和状态，接着这个台阶我们再一次回到Web，进一步把Web开发这个3D点云丰富一下。
...
当相应功能匹配上之后，Vercel 提供的后端运行环境中相应的JS功能代码就开始运行，那么问题来了，任务交到了程序的手上，程序怎么知道具体要干什么呢？
...
> HTTP 是通信格式；API 是双方约定；`req`、`res` 是程序为了处理 HTTP 而使用的变量或对象。

首先一个HTTP Message往往包含以下几个部分：
...
```
      ...
      │       │       │         │       │       │
      ▼       ▼       ▼         ▼       ▼       ▼
   Request  Headers   Body     Status   Headers   Body
     Line                       Line
      │
      ├── Method
      │
...
然而，仅仅是 HTTP Message 是不足以让程序执行的，因为只有格式而没有内容，会导致程序没有执行的对象（大多数需要），所以具体要有什么内容呢？就依靠API来约定。

所以HTTP和API的关系可以理解成：

> HTTP 提供格式容器，API 规定每个字段的内容，以及如何使用HTTP中的字段

最后，根据API的约定，部署在后端服务器上的JS代码接收相应的变量，并按照相应的约定操作相关的数据，最后返回约定好的数据内容和数据格式。

始终牢记：最终流传在光缆中的内容（准确说是TCP负载）始终是HTTP Message
